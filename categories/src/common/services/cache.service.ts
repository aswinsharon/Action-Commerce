...
export class CacheService {
    ...
    /**
     * Generate standardized category cache key
     */
    ...
    /**
     * Generate standardized category list cache key
     */
    generateCategoryListKey(page: number = 1, limit: number = 10, filters?: any): string {
        const filterStr = filters ? JSON.stringify(filters) : '';
        return `categories:list:${page}:${limit}:${Buffer.from(filterStr).toString('base64')}`;
    }

    /**
     * Generate standardized category tree cache key
     */
    generateCategoryTreeKey(): string {
